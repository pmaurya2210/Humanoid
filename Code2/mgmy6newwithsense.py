...
# ─────────────────────────────────────────────
#  OPTIONAL SENSOR IMPORTS (only on Raspberry Pi)
# ─────────────────────────────────────────────
try:
    import lgpio
    import numpy as np
...
class ThinkingSpinner(QtWidgets.QWidget):
    ...
    def paintEvent(self, _):
        ...
        arcs = [
            (72, 4, 200,   0, 220, 255,  1.00,   0),
            (50, 3, 160,   0, 255, 160, -1.45,  40),
            (30, 2, 120, 140,  80, 255,  2.10,  80),
        ]
...
# ══════════════════════════════════════════════════════
#  SENSOR DISTANCE WIDGET  — live radar-style readout
# ══════════════════════════════════════════════════════
class SensorDistanceWidget(QtWidgets.QWidget):
    """Displays live ultrasonic distance with an animated proximity bar."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedSize(210, 80)
        self.setAttribute(QtCore.Qt.WA_TranslucentBackground)
        self._distance   = None   # float cm or None
        self._threshold  = 20     # cm trigger threshold
        self._alert      = False  # True when person in range
        self._pulse      = 0.0
    ...
    def set_distance(self, dist_cm):
        self._distance = dist_cm
        self._alert    = (dist_cm is not None and dist_cm < self._threshold)
    ...
    def paintEvent(self, _):
        ...
        # background pill
        p.setBrush(QtGui.QColor(0, 30, 50, 140))
        p.setPen(QtCore.Qt.NoPen)
        p.drawRoundedRect(0, 0, w, h, 8, 8)

        # title
        p.setFont(QtGui.QFont("Courier New", 7, QtGui.QFont.Bold))
        title_col = QtGui.QColor(0, 130, 180, 140)
        p.setPen(title_col)
        p.drawText(QtCore.QRect(8, 4, w - 16, 14), QtCore.Qt.AlignLeft, "ULTRASONIC SENSOR")

        # border
        pulse_a = int(60 + 40 * math.sin(self._pulse)) if self._alert else 40
        border_col = QtGui.QColor(255, 80, 80, pulse_a) if self._alert else QtGui.QColor(0, 160, 220, 40)
        pen = QtGui.QPen(border_col); pen.setWidth(1)
        p.setPen(pen); p.setBrush(QtCore.Qt.NoBrush)
        p.drawRoundedRect(0, 0, w - 1, h - 1, 8, 8)

        if self._distance is None:
            p.setFont(QtGui.QFont("Courier New", 9))
            p.setPen(QtGui.QColor(0, 100, 140, 120))
            p.drawText(QtCore.QRect(0, 18, w, 28), QtCore.Qt.AlignCenter, "NO ECHO")
        else:
            # distance value
            p.setFont(QtGui.QFont("Courier New", 18, QtGui.QFont.Bold))
            val_col = QtGui.QColor(255, 100, 100, 230) if self._alert else QtGui.QColor(0, 220, 255, 220)
            p.setPen(val_col)
            p.drawText(QtCore.QRect(8, 14, 120, 34), QtCore.Qt.AlignVCenter | QtCore.Qt.AlignLeft,
                       f"{self._distance:.0f}")
            p.setFont(QtGui.QFont("Courier New", 8))
            p.setPen(QtGui.QColor(0, 150, 200, 140))
            p.drawText(QtCore.QRect(8, 42, 80, 14), QtCore.Qt.AlignLeft, "cm")

            # proximity bar (0–200 cm range, clipped)
            bar_x, bar_y, bar_w, bar_h = 8, 58, w - 16, 8
            p.setBrush(QtGui.QColor(0, 40, 60, 100))
            p.setPen(QtCore.Qt.NoPen)
            p.drawRoundedRect(bar_x, bar_y, bar_w, bar_h, 4, 4)

            fill = max(0.0, min(1.0, 1.0 - (self._distance / 200.0)))
            if fill > 0:
                if self._alert:
                    pulse_boost = 0.15 * math.sin(self._pulse)
                    fill = min(1.0, fill + pulse_boost)
                    bar_col = QtGui.QColor(255, 80, 80, 210)
                else:
                    bar_col = QtGui.QColor(0, 200, 255, 180)
                p.setBrush(bar_col)
                p.drawRoundedRect(bar_x, bar_y, int(bar_w * fill), bar_h, 4, 4)

            # ALERT label
            if self._alert:
                p.setFont(QtGui.QFont("Courier New", 7, QtGui.QFont.Bold))
                al_a = int(180 + 75 * math.sin(self._pulse))
                p.setPen(QtGui.QColor(255, 100, 100, al_a))
                p.drawText(QtCore.QRect(130, 14, 72, 34),
                           QtCore.Qt.AlignVCenter | QtCore.Qt.AlignRight, "NEAR!")


# ══════════════════════════════════════════════════════
#  LISTEN WINDOW  — fullscreen popup
# ══════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════
#  SENSOR RECOGNITION WINDOW  — shown during auto-detect
# ══════════════════════════════════════════════════════
class SensorRecognitionWindow(QtWidgets.QDialog):
    """Full-screen overlay shown when the ultrasonic sensor detects someone."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("AURA — Sensor Detection")
        self.setWindowFlags(
            QtCore.Qt.Window |
            QtCore.Qt.FramelessWindowHint |
            QtCore.Qt.WindowStaysOnTopHint
    ...
    def _build_ui(self):
        ...
        top = QtWidgets.QLabel("◉  AURA  —  SENSOR MODE  —  PERSON DETECTED")
        top.setAlignment(QtCore.Qt.AlignCenter)
        top.setStyleSheet("""
            color: rgba(255,120,60,180); font-family: 'Courier New';
            font-size: 13px; font-weight: bold; letter-spacing: 6px;
            padding-top: 50px;
        """)
        root.addWidget(top)
        ...
        # main status
        self.status_lbl = QtWidgets.QLabel("SCANNING...")
        self.status_lbl.setAlignment(QtCore.Qt.AlignCenter)
        self.status_lbl.setWordWrap(True)
        self.status_lbl.setFixedWidth(760)
        self.status_lbl.setMinimumHeight(52)
        self.status_lbl.setStyleSheet("""
            color: rgba(255,160,60,240); font-family: 'Courier New';
            font-size: 30px; font-weight: bold; letter-spacing: 8px;
        """)
        fx = QtWidgets.QGraphicsDropShadowEffect()
        fx.setOffset(0, 0); fx.setBlurRadius(32)
        fx.setColor(QtGui.QColor(255, 140, 40, 140))
        self.status_lbl.setGraphicsEffect(fx)
        root.addWidget(self.status_lbl, alignment=QtCore.Qt.AlignCenter)
        root.addSpacing(14)

        # greeting / result line
        self.result_lbl = QtWidgets.QLabel("INITIALIZING FACE RECOGNITION")
        self.result_lbl.setAlignment(QtCore.Qt.AlignCenter)
        self.result_lbl.setWordWrap(True)
        self.result_lbl.setFixedWidth(760)
        self.result_lbl.setMinimumHeight(52)
        self.result_lbl.setStyleSheet("""
            color: rgba(0,210,255,200); font-family: 'Courier New';
            font-size: 13px; letter-spacing: 4px;
        """)
        root.addWidget(self.result_lbl, alignment=QtCore.Qt.AlignCenter)
        root.addSpacing(30)

        # proximity readout inside the window
        self.dist_lbl = QtWidgets.QLabel("DISTANCE: -- cm")
        self.dist_lbl.setAlignment(QtCore.Qt.AlignCenter)
        self.dist_lbl.setStyleSheet("""
            color: rgba(255,100,80,180); font-family: 'Courier New';
            font-size: 14px; letter-spacing: 3px;
        """)
        root.addWidget(self.dist_lbl, alignment=QtCore.Qt.AlignCenter)
        root.addSpacing(40)

        bar = _ScanBar()
        bar.setFixedSize(480, 7)
        root.addWidget(bar, alignment=QtCore.Qt.AlignCenter)
        root.addSpacing(44)

        # Dismiss button
        btn_dismiss = QtWidgets.QPushButton("  ✕   DISMISS")
        btn_dismiss.setFixedSize(240, 50)
        btn_dismiss.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        btn_dismiss.setFocusPolicy(QtCore.Qt.NoFocus)
        btn_dismiss.setStyleSheet("""
            QPushButton {
                background: rgba(40,15,5,200);
                border: 1px solid rgba(255,120,60,160);
                border-left: 4px solid rgba(255,140,60,255);
                border-radius: 8px; color: rgba(255,150,80,230);
                font-family: 'Courier New'; font-size: 14px;
                font-weight: bold; letter-spacing: 3px;
            }
            QPushButton:hover {
                background: rgba(255,100,40,180);
                color: rgba(255,255,255,255);
            }
        """)
        btn_dismiss.clicked.connect(self.close)
        root.addWidget(btn_dismiss, alignment=QtCore.Qt.AlignCenter)
        root.addStretch()

    def paintEvent(self, _):
        p = QtGui.QPainter(self)
        p.setRenderHint(QtGui.QPainter.Antialiasing)
        p.fillRect(self.rect(), QtGui.QColor(5, 2, 1, 248))
        g1 = QtGui.QRadialGradient(self.width()*0.5, self.height()*0.5, self.width()*0.5)
        g1.setColorAt(0.0, QtGui.QColor(40, 10, 0, 50))
        g1.setColorAt(1.0, QtGui.QColor(0,  0, 0,  0))
        p.fillRect(self.rect(), g1)
        pen = QtGui.QPen(QtGui.QColor(255, 120, 40, 5))
        pen.setWidth(1); p.setPen(pen)
        for y in range(0, self.height(), 4):
            p.drawLine(0, y, self.width(), y)
        for bw, ba in [(12, 4), (6, 10), (2, 30), (1, 80)]:
            pen2 = QtGui.QPen(QtGui.QColor(255, 120, 40, ba))
            pen2.setWidth(bw); p.setPen(pen2); p.setBrush(QtCore.Qt.NoBrush)
            p.drawRect(bw//2, bw//2, self.width()-bw, self.height()-bw)
    ...
    def set_distance(self, dist_cm):
        if dist_cm is None:
            self.dist_lbl.setText("DISTANCE: -- cm")
        else:
            self.dist_lbl.setText(f"DISTANCE: {dist_cm:.1f} cm")


# ══════════════════════════════════════════════════════
#  SENSOR THREAD  — wraps ultrasonic + face recognition
# ══════════════════════════════════════════════════════
class SensorThread(QtCore.QObject):
    """
    Runs the HC-SR04 loop and face recognition session in background threads.
    Emits Qt signals to update the GUI safely from the main thread.
    """
    # Signals (all cross-thread safe)
    distance_updated   = QtCore.Signal(object)   # float or None
    person_detected    = QtCore.Signal(float)    # distance_cm at trigger
    recognition_result = QtCore.Signal(str)      # greeted name or "Unknown"
    ...
    def _init_gpio(self):
        ...
        try:
            ...
            lgpio.gpio_claim_output(self._h, self.TRIG)
            lgpio.gpio_claim_input(self._h,  self.ECHO)
            return True
        except Exception as e:
            self.status_updated.emit(f"GPIO ERROR: {e}")
            return False
    ...
    def _get_distance(self):
        try:
            ...
            while lgpio.gpio_read(self._h, self.ECHO) == 0 and time.time() < deadline:
                start = time.time()
            if time.time() >= deadline:
                return None
            deadline = time.time() + self.SENSOR_TIMEOUT
            while lgpio.gpio_read(self._h, self.ECHO) == 1 and time.time() < deadline:
                end = time.time()
            if time.time() >= deadline:
                return None
    ...
    def _load_models(self):
        ...
        try:
            ...
            dataset  = self.project_dir / "data"
            modelFile  = str(assets / "res10_300x300_ssd_iter_140000.caffemodel")
            configFile = str(assets / "deploy.prototxt")

            self._net = cv2.dnn.readNetFromCaffe(configFile, modelFile)
            self.status_updated.emit("DNN FACE DETECTOR LOADED")

            face_data, labels = [], []
            class_id = 0
            for file in dataset.glob("*.npy"):
                self._names[class_id] = file.stem
            ...
            if not face_data:
                self.status_updated.emit("NO TRAINING DATA FOUND")
                return False

            face_dataset = np.concatenate(face_data, axis=0)
            face_labels  = np.concatenate(labels,    axis=0).reshape(-1, 1)
            self._trainset = np.concatenate((face_dataset, face_labels), axis=1)
            ...
            lbph_faces, lbph_ids = [], []
            for file in dataset.glob("*.npy"):
                cid  = [k for k, v in self._names.items() if v == file.stem][0]
                data = np.load(file)
                for img in data:
                    ...
                    lbph_ids.append(cid)
            if lbph_faces:
                self._lbph.train(lbph_faces, np.array(lbph_ids))

            self._loaded = True
            self.status_updated.emit(f"MODELS READY  ·  {class_id} PERSON(S) LOADED")
            return True
        except Exception as e:
            self.status_updated.emit(f"MODEL LOAD ERROR: {e}")
            return False

    # ── sensor loop (runs in background thread) ───────
    def _sensor_loop(self):
        if not self._init_gpio():
            self.status_updated.emit("SENSOR UNAVAILABLE — GPIO NOT FOUND")
            return
        ...
        self.status_updated.emit("SENSOR ACTIVE  ·  WATCHING FOR APPROACH")

        while self._running:
            dist = self._get_distance()
            self.distance_updated.emit(dist)

            if dist is not None:
                cooldown_over  = (time.time() - self._last_trigger) > self.SENSOR_COOLDOWN
                rec_idle       = (self._rec_thread is None or not self._rec_thread.is_alive())

                if dist < self.THRESHOLD_CM and cooldown_over and rec_idle:
                    self._last_trigger = time.time()
                    self.person_detected.emit(dist)
                    self._rec_thread = threading.Thread(
                        target=self._recognition_session, daemon=True)
                    self._rec_thread.start()

            time.sleep(0.3)

        self._cleanup_gpio()
        self.status_updated.emit("SENSOR STOPPED")

    # ── recognition session (runs in separate thread) ─
    def _recognition_session(self):
        self.session_started.emit()
        self.status_updated.emit("CAMERA ON  ·  RUNNING RECOGNITION")

        try:
            cap = cv2.VideoCapture(0)
            if not cap.isOpened():
                self.status_updated.emit("CAMERA ERROR — CANNOT ACCESS WEBCAM")
                self.session_ended.emit()
                return

            spoken = set()
            end_t  = time.time() + self.RECOGNITION_TIME
            ...
            while time.time() < end_t:
                ...
                for i in range(detections.shape[2]):
                    ...
                    label, conf_val = self._lbph.predict(gray)
                    pred_name = (self._names.get(label, "Unknown")
                                 if conf_val < self.LBPH_THRESHOLD else "Unknown")

                    if pred_name != "Unknown" and pred_name not in spoken:
                        subprocess.Popen(['espeak', '-s', '150',
                                          f"Namaskar {pred_name}. How may I assist you"])
                        spoken.add(pred_name)
                        self.recognition_result.emit(pred_name)
                        self.status_updated.emit(f"GREETED: {pred_name.upper()}")

            cap.release()
        except Exception as e:
            self.status_updated.emit(f"RECOGNITION ERROR: {e}")

        self.session_ended.emit()
        self.status_updated.emit("CAMERA OFF  ·  SESSION COMPLETE")
...
class AuraMain(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self._sq               = None
        self._sensor_thread    = None   # SensorThread QObject
        self._sensor_running   = False
        self._sensor_rec_win   = None   # SensorRecognitionWindow
    ...
    def _make_left_info(self):
        ...
        # ── Sensor status dot (dynamic) ───────────────
        self._sensor_dot = BlinkDot(
            "SENSOR OFF" if not SENSOR_AVAILABLE else "SENSOR READY",
            QtGui.QColor(120, 120, 120) if not SENSOR_AVAILABLE else QtGui.QColor(0, 200, 255)
        )
        vl.addWidget(self._sensor_dot)

        # ── Sensor distance widget ────────────────────
        vl.addSpacing(8)
        self._dist_widget = SensorDistanceWidget()
        vl.addWidget(self._dist_widget)
    ...
    def _make_right_buttons(self):
        ...
        # ── NEW sensor button ─────────────────────────
        self.btn_sensor  = GlowButton("◉   SENSOR MODE",       QtGui.QColor(255, 140, 0))
        self.btn_stop    = GlowButton("⏹   STOP LISTENING",   QtGui.QColor(255, 80, 80))
        self.btn_exit    = GlowButton("✕   EXIT",              QtGui.QColor(255, 50, 70))

        for b in [self.btn_start, self.btn_train, self.btn_manage,
                  self.btn_listen, self.btn_sq, self.btn_sensor,
                  self.btn_stop, self.btn_exit]:
            b.setFixedWidth(300)
            vl.addWidget(b)

        if not SENSOR_AVAILABLE:
            self.btn_sensor.setEnabled(False)
            self.btn_sensor.setToolTip("lgpio / opencv not available on this platform")
        ...
        self.btn_sensor.clicked.connect(self._toggle_sensor)
    ...
    def _start_sensor(self):
        if not SENSOR_AVAILABLE:
            QtWidgets.QMessageBox.warning(
                self, "Sensor Unavailable",
                "lgpio and/or OpenCV not found.\n"
                "Please install them on the Raspberry Pi:\n\n"
                "  pip install lgpio opencv-python")
            return

        project_dir = os.path.dirname(os.path.abspath(__file__))

        # Create the QObject sensor controller on the main thread
        self._sensor_thread = SensorThread(project_dir)

        # Wire signals → GUI slots (all called in main thread via Qt queued connection)
        self._sensor_thread.distance_updated.connect(self._on_distance_update)
        self._sensor_thread.person_detected.connect(self._on_person_detected)
        self._sensor_thread.recognition_result.connect(self._on_recognition_result)
        self._sensor_thread.session_started.connect(self._on_sensor_session_start)
        self._sensor_thread.session_ended.connect(self._on_sensor_session_end)
        self._sensor_thread.status_updated.connect(self._on_sensor_status)

        self._sensor_running = True
        self.btn_sensor.setText("◉   SENSOR ON  [STOP]")
        self.btn_sensor.accent = QtGui.QColor(255, 60, 60)
        self.btn_sensor._apply_style(False)
        self._sensor_dot.set_label("SENSOR ACTIVE")
        self._sensor_dot.set_color(QtGui.QColor(255, 140, 40))
        self._status("SENSOR MODE ACTIVE  ·  WATCHING FOR APPROACH", "rgba(255,160,60,220)")
    ...
    def _stop_sensor(self):
        ...
        self._sensor_running = False
        self.btn_sensor.setText("◉   SENSOR MODE")
        self.btn_sensor.accent = QtGui.QColor(255, 140, 0)
        self.btn_sensor._apply_style(False)
        self._sensor_dot.set_label("SENSOR OFF")
        self._sensor_dot.set_color(QtGui.QColor(120, 120, 120))
        self._dist_widget.set_distance(None)

        if self._sensor_rec_win and self._sensor_rec_win.isVisible():
            self._sensor_rec_win.close()
            self._sensor_rec_win = None

        self._status("SENSOR STOPPED", "rgba(255,80,80,220)")
        QtCore.QTimer.singleShot(2000, self._status_reset)

    # ── sensor signal handlers (run in main thread) ───
    def _on_distance_update(self, dist):
        """Update the distance widget every ~0.3 s."""
        self._dist_widget.set_distance(dist)
        if self._sensor_rec_win and self._sensor_rec_win.isVisible():
            self._sensor_rec_win.set_distance(dist)

    def _on_person_detected(self, dist_cm):
        """Show the sensor recognition overlay when someone is close."""
        self._status(f"PERSON DETECTED AT {dist_cm:.1f} cm  ·  LAUNCHING RECOGNITION",
                     "rgba(255,140,40,240)")
        # Open or reuse the recognition overlay
        if self._sensor_rec_win is None or not self._sensor_rec_win.isVisible():
            self._sensor_rec_win = SensorRecognitionWindow(self)
        self._sensor_rec_win.set_status("SCANNING FACE...")
        self._sensor_rec_win.set_result("INITIALIZING RECOGNITION  ·  PLEASE LOOK AT THE CAMERA")
        self._sensor_rec_win.set_distance(dist_cm)
        self._sensor_rec_win.show()
        self._sensor_rec_win.raise_()

    def _on_recognition_result(self, name):
        """Update overlay when a face is matched."""
        self._status(f"GREETED: {name.upper()}", "rgba(0,255,160,230)")
        if self._sensor_rec_win and self._sensor_rec_win.isVisible():
            self._sensor_rec_win.set_status(f"NAMASKAR, {name.upper()}!")
            self._sensor_rec_win.set_result(
                f"IDENTITY CONFIRMED  ·  WELCOME {name.upper()}  ·  HOW MAY I ASSIST YOU")

    def _on_sensor_session_start(self):
        if self._sensor_rec_win and self._sensor_rec_win.isVisible():
            self._sensor_rec_win.set_status("RECOGNIZING...")

    def _on_sensor_session_end(self):
        """Auto-close the overlay after a short delay."""
        if self._sensor_rec_win and self._sensor_rec_win.isVisible():
            self._sensor_rec_win.set_status("SESSION COMPLETE")
            self._sensor_rec_win.set_result("CAMERA OFF  ·  RETURNING TO WATCH MODE")
            QtCore.QTimer.singleShot(3000, self._close_sensor_rec_win)
        self._status("SENSOR ACTIVE  ·  WATCHING FOR NEXT APPROACH", "rgba(255,160,60,220)")

    def _on_sensor_status(self, text):
        self._status(text, "rgba(255,160,60,220)")

    def _close_sensor_rec_win(self):
        if self._sensor_rec_win and self._sensor_rec_win.isVisible():
            self._sensor_rec_win.close()

    # ══════════════════════════════════════════════════
    #  ORIGINAL BUTTON ACTIONS (unchanged)
    # ══════════════════════════════════════════════════
    def _run_start(self):
    ...
    def _run_exit(self):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            # Clean up sensor thread before exit
            if self._sensor_running:
                self._stop_sensor()
            if self._stt_process and self._stt_process.state() != QtCore.QProcess.NotRunning:
                self._stt_process.kill()
