...
class AuraMain(QtWidgets.QMainWindow):
    ...
    def _listen_worker(self):
        """Background thread: captures mic audio and transcribes with Vosk."""
        ...
        RATE        = 16000
        CHUNK       = 4000
        SILENCE_MAX = int(RATE / CHUNK * 4)   # ~4 seconds silence = done
        ...
        try:
            ...
            # auto-find the Lenovo webcam mic (card 2) or any input device
            device_index = None
            ...
            while not self._listen_stop_flag:
                ...
                if recognizer.AcceptWaveform(data):
                    ...
                    if text:
                        result_text += (" " if result_text else "") + text
                        got_speech = True
                        silence_chunks = 0
                else:
            ...
            # flush final result
            if not self._listen_stop_flag:
                res = json.loads(recognizer.FinalResult())
                tail = res.get("text", "").strip()
                if tail:
                    result_text += (" " if result_text else "") + tail
    ...
    def _stop_listening(self):
        """Stop button — set flag, worker thread will exit on next chunk."""
