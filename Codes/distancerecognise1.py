...
TRIG           = 23
ECHO           = 24
THRESHOLD_CM   = 20   # Trigger recognition when someone is within 20 cm
SENSOR_COOLDOWN = 5   # Seconds before sensor can trigger again
SENSOR_TIMEOUT  = 0.05
...
# ─────────────────────────────────────────────
USE_LBPH         = True
RECOGNITION_TIME = 8    # Seconds to run camera recognition per trigger
LBPH_THRESHOLD   = 150  # Lower = stricter matching
...
for file in dataset_path.glob("*.npy"):
    ...
    data_item = np.load(file).reshape(np.load(file).shape[0], -1) if False else np.load(file)
...
def run_recognition_session():
    """
    Opens the webcam, runs face recognition for RECOGNITION_TIME seconds,
    greets each detected person once, then closes the camera.
    """
    ...
    while time.time() < session_end:
        ...
        for i in range(detections.shape[2]):
            ...
            if pred_name != "Unknown" and pred_name not in spoken_names:
                speak(f"Namaste {pred_name}. Welcome to Omotec!")
                spoken_names.add(pred_name)
                print(f"  → Greeted: {pred_name}")

        # Window hidden — running silently in background
...
# ─────────────────────────────────────────────
#  MAIN LOOP  —  sensor watches, camera wakes
# ─────────────────────────────────────────────
